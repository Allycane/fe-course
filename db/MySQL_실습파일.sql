...
/******************************************************************************************************
    ...
    DML (Data Manuplation) : 생성, 읽기, 수정, 삭제 -> 데이터 기준 (CRUD)
		 - CRUD 작업을 해본적이 있는가 : 데이터베이스 연동해본적이 있는가 : MySQL로 데이터베이스 연동을 진행해보았다
	C Create - Insert
    R Read - Select
    U Update - Update
    D Delete - Delete
...
/******************************************************************************************************
	...
	테이블 생성 형식>
		CREATE TABLE [테이블명] (
			컬럼영 데이터타입 옵션(제약사항, null포함여부...) 
        )
	데이터타입 정리
    분류			타입			크기/형식				설명				사용예
    ---------------------------------------------------------
    정수형		tinyint		1byte				작은 정수값		상탯값(0/1)
				smallint	2byte							카운트
				int			4byte				기본 정수값		일반 정수값
                bigint		8byte				큰 정수값			pk, 주문번호 (숫자로만 줄때 사용)
	실수형		float		4byte				부동소수점			거의 사용안함
				double		8byte				기본 실수값		통계수치
	문자형(고정)	char		고정길이				빠른공간확보(장)		코드값
												공간낭비(단)
    문자형(가변)	varchar		가변길이				가장 많이 사용	이름, 주소, ...
    텍스트		text		~ 64kb				긴 문장 저장		게시글
				longtext	~ 4GB				초대형 텍스트		로그
	바이너리		blob		~64, ~4gb			이미지 파일, 파일	
    날짜			date		yyyy-mm-dd			날짜				생일
				datetime	yyyy-mm-dd + 시간	날짜, 시간...			작업완료
	JSON		JSON		JSON구조, 			API 대회처리
*******************************************************************************************************/
desc employee;

-- emp 테이블 생성
-- emp_id(사번): 4, ename(사원명) : 5, hire_date(입사일):data
-- salary(급여) : 4
create table emp(
	emp_id		char(4),		-- 0.001, 0.002
    ename		varchar(5),
    hire_date	date,
    salary 		int
);
...
/******************************************************************************************************
    ...
    형식 > CREATE TABLE [테이블명]
			AS [서브쿼리];
	VIEW와는 다르게 물리적인 형태로 테이블이 생성되어 데이터로 저장이 됨
*******************************************************************************************************/
-- 2016년에 입사한 사원의 정보를 조회하여 EMPLOYEE_2016 테이블 생성
-- 원하는 데이터가 정상적으로 출력되는지를 먼저 확인하고 create절 연결
create table employee_2016
as
select * from employee where left(hire_date, 4) = '2016';

show tables;
select * from information_schema.tables where table_name = 'employee_2016';

desc employee_2016; -- ✨ 복제본은 원본의 제약사항(KEY값) 복제되지 않는다
desc employee;

-- employee_dept 테이블 생성
-- employee + dept 테이블 조인, dept_id는 하나만 저장
create table employee_dept
as
select e.emp_id, e.emp_name, e.gender, e.hire_date, 
		e.retire_date, d.dept_id, d.dept_name, e.phone, 
        e.email, e.salary, d.unit_id, d.start_date
	from employee e inner join department d on e.dept_id = d.dept_id;

show tables;
select * from information_schema.tables where table_name = 'employee_dept';
desc employee_dept;
select * from employee_dept;

-- 데이터를 출력하지 않고 컬럼만을 출력하는 경우, 테이블 복제 시 테이블 구조만 복제하는 경우
create table emp
as
...
/******************************************************************************************************
    ...
    형식 > INSERT INTO [테이블명](컬럼리스트...) -- 컬럼리스트는 생략이 가능하다
			VALUE(데이터1, 데이터2...)
...
insert into emp(emp_name, -- char(5)
                ...
                email, -- varchar(50)
                salary) -- null 허용
		value('이순신', 'S0002', null, 'M', curdate(), null, 'SYS', '010-1234-1234', 'hong@naver.com', null);
        
-- 컬럼리스트 생략 시 테이블 구조를 기준으로 insert가 진행됨
desc emp;
insert into emp()
		value('S0003', '김유정', null, 'F', curdate(), null, 'SYS', '010-1234-1234', 'hong@naver.com', null);

select * from emp;

-- null 값이 허용되는 컬럼은 생략이 가능하다
insert into emp(emp_id, -- char(5)
                ...
                phone, -- 하이픈 포함 13자리
                email) -- null 허용
		value('S0005', '김삼순', 'F', curdate(), 'SYS', '010-1234-1234', 'hong@naver.com');
select * from emp;

/******************************************************************************************
	 DDL 언어에 포함되는 데이터 절삭 (Truncate :: 원본 데이터가 영구적으로 삭제됨)
...
insert into emp(gender, eid, ename, hire_date)
		values('F', 'S003', '김유정', curdate()); -- hire_date는 datetime 타입이 들어가야 하므로 시분초가 0으로 출력됨
insert into emp(gender, eid, ename, hire_date, salary)
		values('F', 'S004', '송지우', now(), 1000); 
select * from emp;

/******************************************************************************************
	 자동 번호 생성기 : auto_increment
     - 정수형 데이터로 생성됨 (int)
     - 테이블 생성시 옵션 자리에 기술, pk컬럼에 사용
     - 반드시 primary key 제약과 함께 사용
     형식 > CREATE TABLE [테이블명] (
		컬럼명 데이터타입 AUTO_INCREMENT PRIMARY KEY
        ...
     )
     * PK에 꼭 넣으라는 것은 아님
***********************************************************************************************/
-- emp2 생성 : eid(int, 자동번호 생성), ename(varchar(5)), gender(char(1)), hire_date(datetime), salary(int)
-- eid, ename, gender 컬럼은 null을 허용하지 않음
create table emp2(
	eid 		int 		auto_increment		primary key, -- unit + not null
    ename 		varchar(5) 	not null,
    gender 		char(1) 	not null,
    hire_date 	datetime,
    salary 		int
...
show tables;
desc emp2;
select * from information_schema.tables where table_name = 'emp2';

insert into emp2(ename, gender) values('홍길동', 'M');
insert into emp2(ename, gender) values('이순신', 'M');
insert into emp2(ename, gender, hire_date) values('김유정', 'F', now());
insert into emp2(ename, gender, hire_date, salary) values('하유진', 'F', now(), 1000);
select * from emp2;
...
show tables;
desc emp;
-- emp 테이블에 phone(char(13)) 하이픈 포함 컬럼 추가
alter table emp add column phone	char(13);
desc emp;
-- emp 테이블의 phone 컬럼의 데이터타입을 char(20)으로 변경, 데이터 크기를 크게 변경하는 경우는 정상실행
alter table emp modify column phone		char(20);
desc emp;
-- ename 칼럼에 데이터가 존재하는 경우
-- ename 컬럼의 크기를 varchar(2)로 변경, 데이터 크기를 작게 변경하는 경우는 오류발생
-- 데이터 유실이 발생하므로 에러를 발생시킨다
alter table emp modify column ename	varchar(2) not null;
-- ename 칼럼의 크기를 varchar(10)으로 변경
-- 데이터 크기를 크게 변경하는 경우는 정상 실행!
...
-- S001 사번의 phone 데이터 업데이트
update emp set phone = '010-1234-1234' where eid = 'S001';
select * from emp;

-- 모든 사원의 폰 번호를 '010-1111-1234'로 변경
update emp set phone = '010-1111-1234';
select * from emp;
-- 데이터를 입력하였기 때문에 phone에 not null을 적용할 수 있다
desc emp;
alter table emp modify column phone char(13) not null;
desc emp;
...
update emp set email = 'hong@naver.com';
alter table emp modify column email varchar(25) not null;
update emp set email = 'sunsinLee@naver.com' where eid = 'S002'; 
update emp set email = 'jung@naver.com' where eid = 'S003'; 
update emp set email = 'pockemon@naver.com' where eid = 'S004'; 
select * from emp;

-- employee 테이블 복제하여 copy_emp 테이블 생성 - 테이블 복제 : CAS ( CREATE ~ AS ~ SELECT )
show tables;
create table copy_emp
as
select * from employee;
show tables;
select * from information_schema.tables where table_name = 'copy_emp';
desc copy_emp;

-- 홍길동 사원의 급여를 6000으로 수정
select emp_id from copy_emp where emp_name = '홍길동'; -- S0001
update copy_emp set salary = 6000 where emp_id = 'S0001';
select * from copy_emp where emp_name = '홍길동';

-- 안경태 사원의 입사일을 '20210705'로 수정
select emp_id from copy_emp where emp_name = '안경태'; -- S0007
update copy_emp set hire_date = cast('20210705' as date) where emp_id = 'S0007';
...
-- 2) 기존 null데이터를 현재 날짜로 수정
-- 3) 업데이트 완료 후, retire_date를 not null 제약으로 정리
...
-- 부서명이 정보시스템(SYS)인 모든 사원 급여를 20% 증가
select dept_id from department where dept_name = '정보시스템'; -- SYS
...
-- 강우동 사원의 정보 변경
-- 'S0003' 강우동 사원의 영어이름을 'kang', 입사일은 현재 날짜, 부서는 마케팅 'MKT'로 변경
...
-- 트랜젝션 별 업데이트 정의 
-- 트랜젝션 관리 명령어 DTL : commit(작업완료), rollback(작업복원)
-- 1 실시간으로 commit이 진행됨 / 0 rollback으로 기존 데이터를 복구할 수 있음
-- DML 명령어에 영향을 줌 (CRUD), DDL은 관리방식에 상관없이 무조건 autocommit이 진행된다
select @@autocommit;
set autocommit = 0; -- transaction 관리 방식을 수동으로 전환

commit; -- 그 전까지의 작업은 물리적으로 데이터에 반영이 됨, 동시에 새로운 트랜젝션의 시작
select * from emp;

-- 홍길동의 급여를 3000으로 수정
update emp set salary = 3000 where eid = 'S001';
select * from emp; -- commit을 진행하지 않아 아직까진 임시 데이터로 저장됨
-- rollback; -- 트랜젝션을 마지막으로 commit했던 지점 기준
select * from emp; -- 홍길동 사원의 salary가 3000에서 null로 롤백됨

update emp set salary = 3000 where eid = 'S002';
select * from emp;
commit;
rollback;
select * from emp; -- commit한 이후에는 rollback을 진행할 수 없다
...
select @@autocommit; -- 수동으로 트랜젝션 관리
commit;

-- emp 테이블에서 이순신 사원 삭제
select * from emp;
delete from emp where eid = 'S002';
select * from emp; -- commit을 진행하지 않아 보이지만 않도록 비활성화 되어있을 뿐
rollback;
select * from emp;

-- emp 테이블에서 이순신 사원, 홍길동 사원 삭제
delete from emp where eid in ('S001', 'S002');
select * from emp;
commit;

-- emp 테이블에 모든 사원을 삭제
-- truncate 명령어 사용 : DDL - 트랜젝션 상관없이 autocommit이 진행된다
truncate table emp;
select * from emp;
...
set autocommit = 1;
select @@autocommit; -- 트랜젝션 관리 1


/*************************************************************************************************************
	제약사항(Constraints) : 데이터 무결성 원칙을 적용하기 위한 규칙
		- Unique 제약사항 : 중복을 방지하는 제약
        - NOT NULL 제약사항 : null 값을 사용하지 않는 제약
        - Primary 제약사항 : Unique + NOT NULL 제약이 같이 적용된 제약 = AUTO INCREMENT + PRIMARY 함께 사용
        - Foreign 제약사항 : 타 테이블의 기본키를 참조하는 제약 = 기본키와 참조하는 컬럼은 데이터 없이 동일해야 함 = JOIN 연결키를 만들어줌
        - Default 제약사항 : 데이터 입력 시 기본으로 저장되는 데이터를 정의하는 제약
			** 제약사항은 테이블 생성, 수정 시 정의할 수 있다
***************************************************************************************************************/
select @@sql_safe_updates; -- 업데이트 모드 확인
select @@autocommit; -- 트랜젝션 방식 확인
set @@sql_safe_updates = 0;

-- 제약사항 확인
show tables;
desc employee;
select * from information_schema.table_constraints where table_schema = 'hrdb2019'; -- hrdb2019 내의 테이블의 제약정보를 확인

-- emp_const 테이블 생성, 기본 키 제약을 추가 (Primary), 참조키 제약 (Foreign), not null
...
desc emp_const3;
select * from information_schema.tables where table_name = 'emp_const3';
...
-- salary 컬럼에 default 제약 추가, 기본급 : 1000
alter table emp_const modify column salary	int	default 1000;
select * from emp_const;
desc emp_const;

-- hire_date 컬럼에 default 제약 추가 curdate()
alter table emp_const modify column hire_date date default '2026-04-28'; -- 함수는 포함시킬 수 없다 / 문법적 오류
desc emp_const;
...
-- check 제약 : MySQL 8.0(+) 버전 이상부터 사용이 가능하다
desc emp_const;
desc emp_const3;
-- emp_const3 테이블의 hire_date 컬럼은 default 제약 추가 '2026-04-01'
-- salary 컬럼은 not null 제약 변경
select * from emp_const3;
alter table emp_const3 modify column hire_date date default '2026-04-01';
alter table emp_const3 modify column salary int not null;
desc emp_const3;

-- salary 값 입력 시 '3000' 이상의 값만 저장되도록 check 제약사항 추가
...
-- constraint_name = chk_emp_const3_salary : constraint_type = check

select * from emp_const3;
insert into emp_const3(emp_id, emp_name, salary)
	values('S001', '홍길동', 4000); -- check 제약으로 3000 이하의 salary 값을 적용할 수 없음 / 오류 발생

-- emp_const3 테이블에 emp_name 컬럼에 unique 제약 추가
select * from information_schema.table_constraints where table_name = 'emp_const3';
select * from information_schema.key where table_name = 'emp_const3';
-- alter table emp_const3 modify column emp_name varchar(5);
alter table emp_const3 add constraint un_emp_const3_emp_name unique(emp_name);
desc emp_const3;
insert into emp_const3 values('S002', '홍길순', curdate(), 3000);
select * from emp_const3;
-- alter table emp_const3 drop constraint emp_name;
...
-- emp_const2 데이터 삭제
-- delete / truncate
delete from emp_const2; -- 테이블 내 데이터 삭제
-- emp_id 기본키 제약 추가
...
-- department2 테이블에 dept_id 컬럼에 키본키 제약 추가
alter table department2 add constraint pk_department_dept_id primary key(dept_id);
desc department2;
select * from information_schema.key_column_usage where table_name = 'department2';

-- emp_const2 테이블의 dept_id 컬럼 참조제약 추가 --> department2 테이블의 dept_id 컬럼을 참조 = select 시 JOIN 연결키가 됨
desc emp_const2; -- did
...
-- emp_const2 의 부서아이디 컬럼에 참조키 제약 추가
...
-- 컬럼명은 달라도 참조가 가능하지만, 참조하는 컬럼 내에 해당하는 데이터가 없을 경우 에러가 발생한다
-- 참조 데이터를 정확하게 받아와서 사용해야 함
insert into emp_const2(emp_id, emp_name, did) values('S001', '홍길동', 'SYS');
insert into emp_const2(emp_id, emp_name, did) values('S002', '이순신', 'ACC');
select * from emp_const2;

-- department2 테이블의 SYS 부서 삭제 -> emp_const2에서 참조하므로 삭제불가
-- (1) emp_const2 테이블의 sys 참조행을 삭제 혹은 업데이트
delete from emp_const2 where did = 'SYS';
select * from emp_const2;
-- (2) department2 테이블의 sys 데이터 삭제가 가능해진다
delete from department2 where dept_id = 'SYS';
select * from department2;

-- 참조하는 부모 테이블의 컬럼이 변화함에 따라 자식도 함께 적용하도록 옵션 정의
-- ON [DELETE/UPDATE] CASCADE 참조키 제약 정의시 마지막에 추가
-- DELETE : 부모가 삭제되면 참조하고 있는 자식도 자동으로 삭제
-- UPDATE : 부모가 갱신되면 참조하고 있는 자식도 자동으로 갱신
select * from information_schema.table_constraints where table_name = 'emp_const2';
-- emp_const2 테이블의 참조키 제약 삭제
alter table emp_const2 drop constraint fk_emp_const2_did;

-- emp_const2 테이블의 참조키 제약 정의, on delete/update cascade 추가
alter table emp_const2 add constraint fk_emp_const2_did 
	foreign key(did) references department2(dept_id) 
...
-- department2 테이블의 회계부서 (ACC) 이름을 'ABC' 부서로 수정
update department2 set dept_id = 'ABC' where dept_id = 'ACC';
select * from emp_const2;

-- department2 테이블의 'ABC' 부서 삭제
delete from department2 where dept_id = 'ABC';
select * from department2;
select * from emp_const2;

/***************************************************************
	행 번호 : row_number() over() as '별칭'
***************************************************************/
use hrdb2019;
select database();
select row_number() over() as rno, emp_id, emp_name, salary, hire_date from employee where dept_id = 'sys';

-- 정보시스템 부서의 사원들 연봉이 높은 순으로 사번, 사원명, 급여 조회
select row_number() over() as rno, emp_id, emp_name, salary, hire_date from employee where dept_id = 'sys' order by salary desc;

-- row_number와 같이 사용할 경우
select row_number() over(order by salary desc) as rno, emp_id, emp_name, salary, hire_date from employee where dept_id = 'sys';

-- 휴가를 많이 사용한 사원 순으로 조회, 행번호, 사원아이디, 사원명, 휴가일수(총합)
select * from vacation;
select row_number() over(order by sum(duration) desc) as rno, e.emp_id, e.emp_name, sum(duration)
	from employee e inner join vacation v on e.emp_id = v.emp_id group by v.emp_id;
...
-- 사원의 급여가 높은 순으로 정렬
select * from employee order by salary desc;
select rank() over(order by salary desc) as 'srank', emp_id, emp_name, hire_date, salary from employee;

-- 입사일이 빠른 순서대로 정렬
select rank() over(order by hire_date) as 'hrank', emp_id, emp_name, hire_date, salary from employee;

/*****************************************************************
	트리거 생성 : 테이브르이 pk 정의 :: "문자" + "000" + "1"(auto_increment)
*****************************************************************/
...
-- 현재 저장된 값 중 가장 큰 값을 가져와야 순서대로 숫자가 올라가는 형식으로 저장되기 때문 + 중복이 발생하지 않는다
SELECT IFNULL(MAX(CAST(right(mid, 4) AS UNSIGNED)), 0) -- 정수형의 숫자로 변환 : UNSIGNED
INTO max_code
FROM member; 

-- 'M0001' 형식으로 아이디 생성, LPAD(값, 크기, 채워지는 문자형식) : M0001
SET NEW.mid = concat('M', LPAD((max_code+1), 4, '0')); -- LPAD() 0을 채워줌
...
insert into member(name, mdate) values('김유신', now());
insert into member(name, mdate) values('정주교', now());
