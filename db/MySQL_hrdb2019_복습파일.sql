...
select emp_id as 사번, emp_name as 사원명, 
	gender as 성별, hire_date as 입사일, salary as 급여 from employee;

-- 사원테이블의 ID, NAME, GENDER, HDATE, SALARY 컬럼명으로 조회
select emp_id as ID, emp_name as NAME, gender as GENDER, hire_date as HDATE, salary as SALARY
	from employee;
...
select emp_name, gender, hire_date, dept_id, email, salary from employee
	where emp_id = 'S0005';

-- SYS 부서에 속한 모든 사원들을 조회, 단 출력되는 EMP_ID 컬럼은 '사원번호' 별칭으로 조회
select emp_id as 사원번호, emp_name, gender, hire_date, dept_id, email, salary
	from employee
    where dept_id = 'SYS';
...
select emp_id, emp_name, eng_name, ifnull(salary, 0) as salary from employee where dept_id = 'STG';

-- 사원 전체 테이블의 내용을 조회, 단 영어이름이 정해지지 않은 사원들은 '홍길동' 이름으로 치환
select emp_id, emp_name, ifnull(eng_name, '홍길동') as eng_name from employee;

-- MKT 부서의 사원들을 조회, 재직중인 사원들의 RETIRE_DATE 컬럼은 현재 날짜로 치환
select emp_id, emp_name, eng_name, hire_date, ifnull(retire_date, curdate()) as retire_date from employee;
...
select emp_id, emp_name, hire_date, retire_date, salary as '급여'
	from employee
...
select emp_id, emp_name, hire_date as '입사일', salary as '급여' from employee
	where dept_id = 'SYS'
...
select truncate(1234.567, 2) from dual;
-- 사원테이블의 sys 부서 사원들의 사번, 사원명, 부서아이디, 폰번호, 급여, 
-- 보너스(급여의 25%)컬럼을 추가하여 조회, 보너스는 소수점 1자리 출력
select emp_id, emp_name, dept_id, phone, salary, truncate(salary*0.25, 1) as bonus from employee;
...
select emp_id, emp_name, concat(emp_id, '(', emp_name, ')') from employee;

-- 사번, 사원명, 영어이름, 입사일, 폰번호, 급여를 조회
-- 영어이름의 출력형식을 '홍길동/hong' 타입으로 출력
-- 영어이름이 null인 경우에는 'smith'를 기본으로 조회
select emp_id, emp_name, 
	concat(emp_name, ' / ', ifnull(eng_name, 'smith')), 
    hire_date, phone, salary from employee;
...
select sum(ifnull(salary, 0)) from employee;

-- (2) avg(숫자) : 전체 평균을 구하는 함수
-- 사원들의 전체 급여 평균을 조회, 3자리씩 ','로 구분하고 앞에 '$' 표시
-- 2026-04-21 기준 급여가 null이면 0으로 기본값 정의, 소수점은 절삭
select concat('$', format(floor(avg(ifnull(salary, 0))), 0)) as avg_salary from employee;

-- 정보시스템(sys) 부서 전체의 급여 총액과 평균을 조회
-- 3자리 구분, 마지막 '만원' 표시
select concat(format(sum(salary), 0), ' 만원') as '급여 총액',
		concat(format(floor(avg(salary)), 0), ' 만원') as '급여 평균' from employee where dept_id = 'SYS';
